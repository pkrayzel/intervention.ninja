...
def validate_input_arguments(body, context):
    ...
    if template not in constants.SUPPORTED_TEMPLATES:
        logger.warning(f'Template is not supported: {template}. Should be one of: {constants.SUPPORTED_TEMPLATES}')
...
def send_email(arguments):
    mail.send_email(arguments.email, arguments.template)
    logger.info(f'Email with template {arguments.template} has been successfully sent to address: {arguments.email}')
